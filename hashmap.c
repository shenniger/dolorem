 ...
 *
 * Further modified by Simon Henniger.
 *
 * Licence: CC0
 */
#include "hashmap.h"
...
/* Changed by Simon Henniger */

unsigned int hash_of_string(const char *keystring) {
...
unsigned int hashmap_hash_int(hashmap_map *m, const char *keystring) {
  return hash_of_string(keystring) % m->table_size;
}
...
extern int hashmap_get_ptr(map_t in, const char *key, any_t **arg) {
  int curr;
  int i;
  hashmap_map *m;
